...
`
    ...
    def get_occurrence(self, date):
        ...
        start = timezone.make_naive(self.start, timezone.get_current_timezone())
        self.start = timezone.make_aware(start, timezone.get_current_timezone())
    ...
    def _get_occurrence_list(self, start, end):
        ...
        if self.rule is not None:
            ...
            rule = self.get_rrule_object()
            o_starts = rule.between(start, end, inc=True) or \
                rule.between(start - difference, end - difference,
                             inc=True) \
                or rule.between(start - (difference/2),
                                end - (difference/2), inc=True)
